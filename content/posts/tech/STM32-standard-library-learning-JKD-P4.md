...
- MCU
- Keil
description: "本博客讲解了STM32开发在Keil中新建工程，首先讲解了建立STM32工程的复杂结构，包括寄存器方式、标准库方式和HAL库方式。然后详细介绍了使用库函数方式建立工程的步骤，包括下载固件库文件、添加启动文件和内核寄存器描述文件等。最后演示了如何在Keil软件中添加必要文件并创建主函数。"
weight: 
slug: ""
draft: false # 是否为草稿
comments: false
